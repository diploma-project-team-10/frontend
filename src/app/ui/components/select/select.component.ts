...
export class TCSelectComponent
  ...
  ngOnInit() {
    this.currentBgColor =
      this.bgColor instanceof Array ? this.bgColor[0] : this.bgColor;

    this.currentBorderColor =
      this.borderColor instanceof Array
        ? this.borderColor[0]
        : this.borderColor;

    this.currentColor =
      this.color instanceof Array ? this.color[0] : this.color;
    ...
    this.ngZone.runOutsideAngular(() => {
      fromEvent(window.document, 'click')
        .pipe(
          filter(
            (event: MouseEvent) => this.opened && this.isClickOutside(event)
          ),
  ...
  getOptions(options: IOption[]): IOption[] {
    return options.filter((option: IOption) => {
      return option.label
        .toString()
        .toLowerCase()
        .startsWith(this.filterForm.value.toString().toLowerCase());
    });
  }
    ...
    if (this.options && this.options.length) {
      this.selectedOptions = this.options.filter(option => {
        return !this.multiple
          ? option.value === value
          : this.includeSelected(value, option.value);
      });
    }
  }
  ...
  includeSelected(selected: string | string[], value: string) {
    return (
      Array.isArray(selected) && selected.length && selected.includes(value)
    );
  }
  ...
  selectOption(opt: IOption, active: boolean) {
    if (this.multiple) {
      ...
      Array.isArray(this.innerValue)
        ? this.writeValue([...this.innerValue, opt.value])
        : null;
    } else {
      if (!active) {
