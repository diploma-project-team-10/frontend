...
@Component({
  ...
  styleUrls: ['./teacher-list.component.scss', '../../../../reference/folder/folder.component.scss'],
})
export class TeacherListComponent extends TCFolderComponent implements OnInit, OnDestroy {
  ...
  async getListRefRecord() {
    this.headerRecord = [
        TeacherFields['fio'],
        TeacherFields['speciality']
    ];
    this.loading = true;
    this.initTable();

    const url = `${environment.apiUrl}/api/project/course/teacher/list` +
  ...
  prepareNavigateLink(id) {
    return ['/vertical/courses/teacher/view', id];
  }
}
