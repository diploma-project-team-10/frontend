...
export class UserService {
  ...
  getAdmin(): Promise<Status> {
    return this.http
      .get(`${environment.apiUrl}/api/auth/user/role/super/admin`)
      .toPromise()
  ...
  async roleAccount(role: String[]): Promise<boolean> {
    ...
    const res = await this.http
      .post<Status>(`${environment.apiUrl}/api/auth/account/islogging`, role)
      .toPromise()
      .then((response) => response as Status)
      .catch();
    return res.status === 1 ? true : false;
  ...
  async getRolesAccount(): Promise<string> {
    const roles = await this.http
      .get(`${environment.apiUrl}/api/auth/user/role`)
      .toPromise()
      .then((response) => response as Status)
      .catch();
